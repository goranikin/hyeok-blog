---
title: Go to the VISION
publishDate: 2025-05-02
description: While I already knew CNN architecture, I have yet to explore Vision Transformers, and other modern image processing models. In this article, I'll delve into ViT, MLP-Mixer, and the concept of 'inductive bias' - topics I'm eager to understand more thoroughly.
...
이 물음에 대해서 ViT와 MLP-Mixer를 공부했더라도 답변하지 못했던 건 매 한가지였을 겁니다. 왜냐하면 두 논문에서도 주요하게 다뤄지는 건 Context size와 이로 인해 생기는 inductive bias의 차이니까요. 오히려 context size 관점에서 벗어나지 못해 더 엉뚱한 이야기를 했을지도 모릅니다. 그러나 MLP-Mixer가 가져다주는 통찰도 있습니다. 바로 inductive bias든 multi-head attention이든 두 대상이 잠재적인 특성을 다룬다는 겁니다. 이와 같은 implicit한 의미들을 각각의 구조에서 이끌어내는 게 현재의 LLM을 다루는 데 중요한 능력이라 느꼈습니다. 결국 LLM은 고전적인 algorithm에 의한 machine learning 기법이 아니니까요. 언어의 근본적 특성부터 시작해 인간이 세상에게서 받는 지각을 처리하는 과정에 대한 이해가 SoTA를 만드는 것 아닌지 감히 생각해봅니다.
